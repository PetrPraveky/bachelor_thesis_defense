...
\begin{minipage}{0.7\textwidth}
	\begin{block}{Jak velké (na kilometry) světy jde vygenerovat?}
		\begin{itemize}
			\item Velikostní omezení světů není, ovšem rozlišení meshe terénu je a přesně stanovené hodnoty. 
...
\begin{minipage}{0.7\textwidth}
	\begin{block}{Jsou data z vygenerovaného datasetu použitelná pro vizuální odometrii?}
		\begin{itemize}
			\item Ano, ale tento výsledek není příliš vypovídající, protože u kvadrokoptér bývá hlavním problémem IMU (Inertil Measurement Unit), nikoli množstvím obrazových příznaků.
		\end{itemize}
	\end{block}
\end{minipage}
