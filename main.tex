...
\begin{column}{0.45\textwidth}
	...
	\item Manual creation of realistic large-scale virtual environments is challenging and time consuming process.
	\item Mapping systems are not tested on large datasets.
	\end{itemize}
\end{column}
\begin{column}{0.55\textwidth}
	\begin{figure}[h]
		\centering%
		\includegraphics[keepaspectratio,height=7cm]{fig/screenshots/desert_oasis_1.jpg}
		\caption{Generated Desert Oasis environment.}
	\end{figure}
\end{column}
...
\begin{column}{0.35\textwidth}
	\begin{itemize}
		\item Design a method of autonomous environment generation from real-life data.
...
\begin{column}{0.65\textwidth}
		\begin{figure}[h]
		\centering%
		\includegraphics[keepaspectratio,height=6cm]{fig/drawio/pipeline/simplified_pipeline.drawio.png}
	\end{figure}
\end{column}
...
\vspace{0.8cm}
\vfill {\footnotesize Height maps were generated using: \url{https://manticorp.github.io/unrealheightmap/}.}
...
\begin{figure}
	%\begin{adjustbox}{max totalsize={1.0\textwidth}{.75\textheight}, center}
	%	\input{./fig/tikz/pipeline_diagram.tex}
	%\end{adjustbox}
	\begin{figure}[h]
		\centering%
		\includegraphics[keepaspectratio,height=8cm]{fig/drawio/pipeline/pipeline.drawio.png}
	\end{figure}
\end{figure}
...
\begin{frame}[t]
\frametitle{Polyline Point Reduction and Spline Interpolation}	
	...
	\begin{figure}
		\hspace*{-1cm}
 		\begin{adjustbox}{scale=1, center}
 			\input{./fig/tikz/douglas_peucker.tex}
 		\end{adjustbox}
 	\end{figure}
\end{column}
...
\begin{column}{0.40\textwidth}
	...
	\hspace*{-2.5cm}
	\includegraphics[keepaspectratio,height=4cm]{fig/pdf/catmull_rom.pdf}	
\end{column}
...
\EnvironmentPair
{fig/screenshots/grand_canyon_1.jpg}
{Generated Grand Canyon environment.}
{fig/screenshots/grand_canyon_real.jpg}
{Grand Canyon captured on Google Maps.}
	
\end{frame}

% todo maybe add the rest of the generated environemnts?
...
{Generated Stu{\dh}lagil Canyon environment}
{fig/screenshots/etretat_cliffs_1.jpg}
{Generated \'{E}tretat Cliffs environment}
...
\begin{column}{0.38\textwidth}
	...
	% todo add maybe here an octomap? or something
\end{column}
...
\begin{minipage}{0.7\textwidth}
	\begin{block}{Jak velké (na kilometry) světy jde vygenerovat?}
		\begin{itemize}
			\item Velikostní omezení světů v pluginu není, ovšem je zde omezení hustoty bodů terénu. 
			\item Pro rozestup bodů $1$m bude největší možný svět:
			\begin{equation*}
				((8129 - 1) * 1)^2 \approx 66.06\text{km}^2\text{ ($8.128\times8.128$ km)}
			\end{equation*}
			\item Omezujícím faktorem při generaci světů je PCG Biome Core plugin, který vyžaduje velkou spotřebu paměti.
			\item Největší svět co jsem generoval byl okolo $10\times10$ km.
		\end{itemize}
	\end{block}
\end{minipage}
...
\centering
\begin{minipage}{0.7\textwidth}
	\begin{block}{Proč 3 ze 4 světů obsahují kaňon?}
		\begin{itemize}
			\item Vygenerované světy obsahují, oázu, útesy a dva kaňony.
			\item Kaňony byly vybrány z důvodu, že v Flight Forge simulátoru jsou převážně světy pložší.
		\end{itemize}
	\end{block}
\end{minipage}
...
\centering
\begin{minipage}{0.7\textwidth}
	\begin{block}{O kolik náročnější by pro uživatele bylo nakreslit splines přímo v editoru?}
		\begin{itemize}
			\item Body a tangenty spline se rozmisťují a upravují manuálně.
			\item Kreslení komplexní spline je časově náročné
			\item Problémem jsou křižovatky u landscape splines.
		\end{itemize}
	\end{block}
\end{minipage}
...
\centering
\begin{minipage}{0.7\textwidth}
	\begin{block}{Jsou data z vygenerovaného datasetu použitelná pro vizuální odometrii?}
		\begin{itemize}
			\item Ano, ale tento výsledek není příliš vypovídající, protože u kvadrokoptér bývá hlavním problémem IMU (Inertial Measurement Unit), nikoli množství obrazových příznaků.
		\end{itemize}
	\end{block}
\end{minipage}
...
\centering
\begin{minipage}{0.7\textwidth}
	\begin{block}{Už funguje odkaz na dataset?}
		\begin{itemize}
			\item Ano
			\item \url{https://mrs.fel.cvut.cz/kucera2026thesis}
		\end{itemize}
	\end{block}
\end{minipage}
